...
class Fragments(object):
    """
    Adapted from the Newsroom dataset toolkit (Grusky et al., 2018).
    Original: https://github.com/lil-lab/newsroom/tree/master

    The novelty() method is added; coverage/density/compression follow the
...
def main():
    ...
    results, raw_metrics = evaluate_batch(preds, sources)
    ...
    out_path = args.pairs.replace(".jsonl", "_extract_results.json")

    with open(out_path, "w") as f:
        json.dump({
            "ids": [str(i) for i in ids],
            "raw_coverage": [float(s) for s in raw_metrics["coverage"]],
            "raw_density": [float(s) for s in raw_metrics["density"]],
            "raw_compression": [float(s) for s in raw_metrics["compression"]],
            "raw_novelty2": [float(s) for s in raw_metrics["novelty2"]],
            "raw_novelty3": [float(s) for s in raw_metrics["novelty3"]],
        }, f, indent=4)
