...
def merge(sentences):
    ...
    for i in range(1, len(sentences)):
        ...
        if re.search(midden_of_sentence_pattern, previous):
            segmentedsentence = merge_adjustment(
                i, sentences, segmentedsentence, current
            )
        ...
        # if odd number of quotations or parentheses  is found in both sentences, merge
            ...
            (len(re.findall(r"[\"'“”]", previous)) % 2 != 0)
            and (len(re.findall(r"[\"'“”]", current)) % 2 != 0)
        ) or (
            (len(re.findall(r"[\(\)]", previous)) % 2 != 0)
            and (len(re.findall(r"[\(\)]", current)) % 2 != 0)
        ):
            ...
            re.search(bacteria_pattern, previous)
            or re.search(other_pattern, previous)
            or re.search(other_num_pattern, previous)
        ):
            segmentedsentence = merge_adjustment(
                i, sentences, segmentedsentence, current
            )
        elif not re.search(text_pattern, current) and not re.search(
            end_pattern, previous
        ):
            segmentedsentence = merge_adjustment(
                i, sentences, segmentedsentence, current
            )
            ...
            j += 1
            segmentedsentence.append(
                {"sentence": current, "span": sentences[i]["span"]}
            )
    return segmentedsentence
...
def segment(full_text, section_header=None):
    ...
    for res_tuple in all_result:
        ...
        sentences.append(
            {"sentence": text, "span": [start, start + text.__len__() - 1]}
        )
        start += text.__len__()
...
if __name__ == "__main__":
    ...
    print(
        re.search(
            p,
            "We \b assessed circulating EPC levels and EPC outgrowth number and function in CRS patients compared to healthy controls, and evaluated whether short-term (18 days) and long-term (52 weeks) EPO therapy improved EPC number and function in patients with CRS.\n      Methods",
        )
    )
