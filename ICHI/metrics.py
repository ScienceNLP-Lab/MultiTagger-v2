import numpy as np
import torch
from sklearn.metrics import average_precision_score, f1_score, roc_auc_score, precision_recall_curve, precision_score, recall_score, classification_report, mean_squared_error, auc
...
def calculate_multilabel_metrics(predictions_logits, predictions, labels, label_list):
	...
	try:
		auprc_micro = average_precision_score(labels_arr, predictions_logits_arr, average='micro')
	except ValueError:
		auprc_micro = 'Value Error'
	try:
		auprc_macro = average_precision_score(labels_arr, predictions_logits_arr, average='macro')
	except ValueError:
		auprc_macro = 'Value Error'

	metrics = {
		'micro f1': f1_micro_average,
	...
	for i, lab in enumerate(label_list):
		...
		try:
			...
			precision, recall, _ = precision_recall_curve(lab_labels, lab_preds)
			metrics[f'{lab} prc'] = auc(recall, precision)
			metrics[f'{lab} mse'] = mean_squared_error(lab_labels, lab_preds)
		except ValueError:
			metrics[f'{lab} auc'] = 'Value Error'
			metrics[f'{lab} prc'] = 'Value Error'
		if roc_auc_macro == 'Value Error':
		...
		try:
			lab_preds = torch.from_numpy(np.transpose(predictions_logits_arr[:, i])).float()
			lab_labels = torch.from_numpy(np.transpose(labels_arr[:, i])).float()
			metrics['ece_l1'] = ece_l1_metric(lab_preds, lab_labels).item()
			metrics['ece_l2'] = ece_l2_metric(lab_preds, lab_labels).item()
			metrics['ece_max'] = ece_max_metric(lab_preds, lab_labels).item()
		except Exception as e:
			print(f'Exception during performance calculation: {e}')
			pass
		...
		if lab in core_collections_list:
			...
			if metrics[f'{lab} prc'] != 'Value Error':
				core_collection_macro_auprc.append(metrics[f'{lab} prc'])
	...
	metrics['core collections auprc'] = (sum(core_collection_macro_auprc) / len(core_collection_macro_auprc))
	## Calculate ECE
