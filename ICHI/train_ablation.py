...
from data_ablation import collate_fn, data_load
...
tokenizer = AutoTokenizer.from_pretrained("allenai/specter2_base", do_lower_case=True, use_fast=True)

def get_warmup_lambda(epoch, warmup_epochs=5, lambda_max=0.3):
		# linear warmup
...
def get_adv_lambda(epoch, max_epoch, base_lambda=0.1):
		"""
		Dynamic adversarial weight schedule (DANN-style).
		λ starts ~0.0 and ramps up to base_lambda.
		"""
		p = epoch / max_epoch
		return base_lambda * (2. / (1. + np.exp(-10 * p)) - 1.)
...
def save_checkpoint(model, optimizer, scaler, scheduler, epoch, step, optimal_thresholds, fpath_base, config):
		# 1. Save model weights safely
		save_file(model.state_dict(), f"{fpath_base}.safetensors")
		
		# 2. Save training state (optimizer + scaler + optional scheduler)
		training_state = {
		...
		if getattr(config, "lr_scheduler", False) and scheduler is not None:
				training_state["lr_scheduler"] = scheduler.state_dict()
		
		torch.save(training_state, f"{fpath_base}_continue_training.pt")


def load_checkpoint(model, optimizer=None, scaler=None, scheduler=None, fpath_base=None, config=None):
		# 1. Load model weights
		print(f"Loading from model: {fpath_base}.safetensors")
		state_dict = load_file(f"{fpath_base}.safetensors")
		model.load_state_dict(state_dict)

		# 2. Load training state
		training_state = torch.load(f"{fpath_base}_continue_training.pt", map_location=config.device)

		# Only load optimizer/scaler/scheduler if objects were passed in
		...
		if (
				getattr(config, "lr_scheduler", False)
		...
		optimal_thresholds = training_state.get("optimal_thresholds", {})

		return model, optimizer, scaler, scheduler, epoch, step, optimal_thresholds


def round_using_optimal_f1_threshold(predictions_logits, labels, list_of_label_names, optimal_thresholds):
	"""
	...
	:param optimal_thresholds: a dict containing optimal threshold cut-offs
	...
	if not optimal_thresholds:
		...
		for i in range(len(list_of_label_names)):
			# require 50 or more instances of each label, otherwise pick 0.5
			if pos_counts[i] < 50:
				optimal_thresholds[list_of_label_names[i]] = 0.5
				predictions_binary[:, i] = np.where(predictions_logits[:, i] > 0.5, 1, 0)
			else:
				...
				optimal_thresholds[list_of_label_names[i]] = max_f1_thresh
				predictions_binary[:, i] = np.where(predictions_logits[:, i] > max_f1_thresh, 1, 0)
	else:
		for i in range(len(list_of_label_names)):
			try:
				predictions_binary[:, i] = np.where(predictions_logits[:, i] > optimal_thresholds[list_of_label_names[i]], 1, 0)
			except KeyError:
				predictions_binary[:, i] = np.where(predictions_logits[:, i] > 0.5, 1, 0)  # uses 0.5 if no key available
	return predictions_binary, optimal_thresholds


def evaluate(model, loss_fn, data, config, batch_num, list_name_=[], epoch=0, name='', optimal_thresholds=None, adv_weights=None):
	model.eval()
	progress = tqdm.tqdm(total=batch_num, ncols=75, desc='{} {}'.format(name, epoch))
	list_name = list_name_.copy()
	sid = []
	running_loss = 0.0
	adv_running_loss = 0.0  # <-- track adversarial loss separately
	...
	mesh_classes = len(data[0].adv_labels)
	print(f"Number of classes for adversarial training layer: {mesh_classes}")

	with torch.no_grad():
		for batch in DataLoader(data, batch_size=config.eval_batch_size, shuffle=False, collate_fn=partial(collate_fn, config=config, tokenizer=tokenizer)):
			model.eval()
			target = batch.labels
			adv_logits = None
			if config.contrastive_loss == 'HeroCon' or config.contrastive_loss == 'WeighCon':
				cl_losses, combine, adv_logits = model(batch, training=False) 
			else:
				combine = model(batch)
			...
			# adversarial loss
			if config.adversarial and adv_logits is not None:
				adv_loss_fn = nn.BCEWithLogitsLoss(pos_weight=adv_weights)
				adv_loss = adv_loss_fn(adv_logits, batch.adv_labels.float())
				adv_running_loss += float(adv_loss.item())
				...
				adv_true_all.append(batch.adv_labels.cpu().numpy())
				adv_prob_all.append(adv_probs)
				adv_pred_all.append((adv_probs > 0.5).astype(np.int32))
	...
	if getattr(config, "adversarial", False) and len(adv_true_all) > 0:
		adv_true = np.concatenate(adv_true_all, axis=0)   # (N, C)
		adv_prob = np.concatenate(adv_prob_all, axis=0)   # (N, C)
		adv_pred = np.concatenate(adv_pred_all, axis=0)   # (N, C)

		# Macro AUC over classes that have both pos & neg
		per_cls_auc = []
		for c in range(adv_true.shape[1]):
			y = adv_true[:, c]
			if y.max() != y.min():  # skip degenerate classes
				per_cls_auc.append(roc_auc_score(y, adv_prob[:, c]))
		
		adv_macro_auc = float(np.mean(per_cls_auc)) if len(per_cls_auc) else None

		# Micro AUC (flattened)
		if adv_true.size and (adv_true.max() != adv_true.min()):
			adv_micro_auc = float(roc_auc_score(adv_true.reshape(-1), adv_prob.reshape(-1)))
		else:
			adv_micro_auc = None
		
		if (adv_pred_all is not None) and len(adv_pred_all) > 0:
			adv_pred = np.concatenate(adv_pred_all, axis=0)
			adv_micro_f1 = float(f1_score(adv_true.reshape(-1), adv_pred.reshape(-1)))

	else:
			adv_macro_auc, adv_micro_auc, adv_micro_f1 = None, None, None

	avg_running_loss = running_loss/len(data)

	if getattr(config, "adversarial", False):
		avg_adv_running_loss = adv_running_loss / len(data)
	else:
		avg_adv_running_loss = None

	if config.optimal_f1_threshold:
		binary_predictions, optimal_thresholds = round_using_optimal_f1_threshold(probability_predictions, true_labels, list_name, optimal_thresholds)
	else:
	...
	return sid, avg_running_loss, avg_adv_running_loss, adv_micro_auc, adv_macro_auc, adv_micro_f1, true_final, preds_final, base_performance_metrics, all_performance_metrics, probability_predictions, binary_predictions, true_labels, optimal_thresholds
...
def train(config):
	...
	train_dataset, val_dataset, test_dataset, list_name, config, adv_weights = data_load(config)  # list_name is list of labels
	...
	if config.save:
		...
		best_model_fpath = os.path.join(log_dir, 'best_model')
		with open(log_fpath, 'w', encoding='utf-8') as w:
			w.write(str(config) + '\n')

	mesh_classes = len(val_dataset[0].adv_labels)	
	if config.contrastive_loss == 'HeroCon' or config.contrastive_loss == 'WeighCon':
		model = BertForMultiLabelClassificationHeroCon(config, num_labels, mesh_classes, clip=config.cl_clipping_val)
	else:
	...
	if config.loss_function == 'bce':
		...
		loss_fn = AsymmetricLossOptimized_LabelSmoothing(num_labels, alpha=config.ls_alpha, gamma_neg=config.ls_gamma_neg, gamma_pos=config.ls_gamma_pos, clip=config.ls_clip)

	best_metric, best_epoch, start_epoch = 0, 0, 0

	if config.continue_training:
		model, optimizer, scaler, scheduler, start_epoch, start_step, best_thresholds = load_checkpoint(model, optimizer, scaler, scheduler, best_model_fpath, config)
		start = False
	...
	for epoch in range(start_epoch, config.max_epoch):
		...
		if config.adversarial:
			adv_loss_fn = nn.BCEWithLogitsLoss(pos_weight = adv_weights)  
			if config.dynamic_adversarial:
				adv_lambda = get_adv_lambda(epoch, config.max_epoch, base_lambda=config.adv_lambda)
			elif config.scheduler_adv:
				adv_lambda = get_warmup_lambda(epoch, warmup_epochs=config.adv_warmup_epoch, lambda_max=config.adv_lambda)
			else:
		...
		dataloader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, collate_fn=partial(collate_fn, config=config, tokenizer=tokenizer))
		for batch_idx, batch in enumerate(dataloader):
			...
			with torch.autocast(device_type='cuda', dtype=torch.float16):
				if config.contrastive_loss == 'HeroCon' or config.contrastive_loss == 'WeighCon':
					cl_losses, prediction, adv_logits = model(batch, training=True, sup_contrastive_mode=config.contrastive_loss, focusing=config.cl_focusing, clipping=config.cl_clipping)
					task_loss = loss_fn(prediction, batch.labels)
					if config.adversarial and epoch >= config.adv_warmup_epoch and adv_logits is not None:
						adv_loss = adv_loss_fn(adv_logits, batch.adv_labels.float())
						# print(adv_loss)
						# print(batch.adv_labels.shape)
						loss = (task_loss + cl_losses[0] + cl_losses[1]).mean()
						loss = loss + adv_lambda*adv_loss
					else:
						loss = (task_loss + cl_losses[0] + cl_losses[1]).mean()

				else: # since we are never going into this branch, ignore updating with adversarial configs for now.
			...
			if ((batch_idx + 1) % iters_to_accumulate == 0) or (batch_idx + 1 == len(dataloader)):
				...
				if config.lr_scheduler == 'linear' or config.lr_scheduler_restarts == 'per_epoch':
					scheduler.step()
			n_iter += 1
			running_loss += loss.item() # monitoring the combined objective (main task + contrastive + adversarial) during training to check optimization stability.
			progress.update(1)
			step += 1
			if step % eval_steps == 0:
				print('\nINFO: Training loss is ', running_loss/step)
				if epoch >= config.eval_after_epoch:
					sid, eval_avg_loss, avg_adv_loss, adv_micro_auc, adv_macro_auc, adv_micro_f1, target_result, valid_result, base_performance_metrics, all_performance_metrics, probability_predictions, binary_predictions, true_labels, optimal_thresholds = evaluate(model, loss_fn, val_dataset, config, val_batch_num, list_name_=list_name, epoch=epoch, name='VALID')
					print('\nINFO: Validation loss is ', eval_avg_loss)
					print(f'\nINFO: Performance is {base_performance_metrics} on evaluation set at step {step}')
					if config.save:
						result = json.dumps({'epoch': str(epoch), 'train_loss': str(running_loss/len(train_dataset)), 'eval_loss': str(eval_avg_loss), 'eval_adv_loss': str(avg_adv_loss), "adv_macro_auc": str(adv_macro_auc), "adv_micro_auc": str(adv_micro_auc), "adv_micro_f1": str(adv_micro_f1), 'performance_metrics': str(all_performance_metrics)})
						...
						if comparison_metric > best_metric:
							...
							if config.bert_model_name == "allenai/specter2_base" and config.bert_adapter:  # adapters don't seem to save using safetensors
								torch.save({'epoch': epoch, 'step': step, 'model_state_dict': dict(model=model.state_dict(), config=config), 'optimizer_state_dict': optimizer.state_dict(), 'scaler_state_dict': scaler.state_dict()}, best_model_fpath)
							else:
								save_checkpoint(best_model, optimizer, scaler, scheduler, epoch, step, optimal_thresholds, best_model_fpath, config)
								print(f"[train] New best model saved at: {best_model_fpath}", flush=True)
							# saves predictions of best model
							path_suffix = config.data_file.split("/")[-1].split(".")[0]
							performance_save_path = os.path.join(config.checkpoint, f"{path_suffix}_val_performance.txt")
							...
							predictions_save_path = os.path.join(config.checkpoint, f'{path_suffix}_val_predictions.csv')
	...
	if config.save:
		best = json.dumps({'best epoch': str(best_epoch)})
		with open(log_fpath, 'a', encoding='utf-8') as w:
			w.write(best + '\n')
	...
	model.to(config.device)
	model.eval()
	sid, eval_avg_loss, avg_adv_loss, adv_micro_auc, adv_macro_auc, adv_micro_f1, target_result, valid_result, base_performance_metrics, all_performance_metrics, probability_predictions, binary_predictions, true_labels, optimal_thresholds = evaluate(model, loss_fn, test_dataset, config, test_batch_num, list_name_=list_name, epoch=epoch, name='TEST', optimal_thresholds=best_thresholds)
	
	if config.save:
		if config.perturbation_test_type:
			path_suffix = config.perturbation_test_type
		else:
			path_suffix = config.data_file.split("/")[-1].split(".")[0]
		
		performance_save_path = os.path.join(config.checkpoint, f'{path_suffix}_test_performance.txt')
		with open(performance_save_path, 'w', encoding='utf-8') as w:
			w.write(str(all_performance_metrics))
		...
		predictions_save_path = os.path.join(config.checkpoint, f'{path_suffix}_test_predictions.csv')
		with open(predictions_save_path, "w", newline='') as f:
			writer = csv.writer(f)
			header = ['pmid', 'named_true_label', 'named_pred', 'logits', 'binary_pred', 'binary_true_label', 'label_names']
			writer.writerow(header)
...
def test(config):
	...
	mesh_classes = len(dataset[0].adv_labels)
	...
	checkpoint_path = os.path.join(config.checkpoint, 'best_model')
	model, _, _, _, start_epoch, start_step, best_thresholds = load_checkpoint(
		model, optimizer=None, scaler=None, scheduler=None,
		fpath_base=checkpoint_path, config=config
	)
	model.to(config.device)
	print(f"[test] Loaded thresholds: {len(best_thresholds) if best_thresholds else 0}", flush=True)
	if best_thresholds:
			print(f"[test] Example thresholds: {list(best_thresholds.items())}", flush=True)
	else:
		print("No thresholds Loaded from checkpoint")
	...
	if config.loss_function == 'bce':
		...
		loss_fn = AsymmetricLossOptimized_LabelSmoothing(num_labels, alpha=config.ls_alpha, gamma_neg=config.ls_gamma_neg, gamma_pos=config.ls_gamma_pos, clip=config.ls_clip)  # default parameter

	eval_batch_num = len(dataset) // config.eval_batch_size + (len(dataset) % config.eval_batch_size != 0)
	sid, eval_avg_loss, avg_adv_loss, adv_micro_auc, adv_macro_auc, adv_micro_f1, target_result, valid_result, base_performance_metrics, all_performance_metrics, probability_predictions, binary_predictions, true_labels, optimal_thresholds = evaluate(model, loss_fn, dataset, config, eval_batch_num, list_name_=list_name, name='TEST', optimal_thresholds=best_thresholds)
	...
	if config.save:
		...
		performance_save_path = os.path.join(config.checkpoint, f'{path_suffix}_{config.train_val_test}_performance.txt')
		with open(performance_save_path, 'w', encoding='utf-8') as w:
			w.write(str(all_performance_metrics))

		print(f"Wrote performance at: {performance_save_path}", flush=True)
		predictions_save_path = os.path.join(config.checkpoint, f'{path_suffix}_{config.train_val_test}_predictions.csv')
		with open(predictions_save_path, "w", newline='') as f:
			writer = csv.writer(f)
			header = ['pmid', 'named_true_label', 'named_pred', 'logits', 'binary_pred', 'binary_true_label', 'label_names']
			writer.writerow(header)
...
if __name__ == '__main__':
	...
	parser.add_argument('--remove_feature', type=str, default='pub_date', help='name a single feature to remove for ablation')
	parser.add_argument('--remove_multiple_features', type=str, default='', help='name multiple features to remove for ablation, separate by semicolon')
	...
	parser.add_argument('--ls_gamma_neg', type=int, default=4, help='control weighting to learn from negative instances')
	parser.add_argument('--ls_gamma_pos', type=int, default=1, help='control weighting to learn from positive instances')
	parser.add_argument('--ls_clip', type=float, default=0.05, help='alpha to control level of label smoothing')
	...
	# Hyperparameters related to adversarial training
	parser.add_argument('--adversarial', type=str, default='', help='whether to use adversarial training')
	parser.add_argument('--advanced_stronger_adv', type=str, default='', help='whether to use drop off linear layer for adversarial training')
	parser.add_argument('--adv_lambda', type=float, default=0.1, help='default adversarial training weight for grl')
	parser.add_argument('--dynamic_adversarial', type=str, default='', help='whether to use dynamic lambda adversarial training')
	parser.add_argument('--use_entity_type_marker', type=str, default='', help='whether to use entity masking training strategy')
	parser.add_argument('--scheduler_adv', type=str, default='', help='whether to use warm up in adversarial training strategy')
	parser.add_argument('--adv_warmup_epoch', type=int, default=0, help='Skip adversary updates until epoch x.')

	# Hyperparameters related to perturbation evaluation
	parser.add_argument('--perturbation_test_type', type=str, default='', help='which perturbation type to choose, 0.3_cui, 0.5_semtype, 1.0_cui, eda_semtype etc.')
	
	parser.add_argument('--augment_with_masked', type=str, default='', help='whether to train with masked abstracts as data augmentation')
	...
	if 'val' in config.train_val_test or 'test' in config.train_val_test:
