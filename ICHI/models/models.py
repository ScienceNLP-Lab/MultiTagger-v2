...
class BertForMultiLabelClassificationHeroCon(nn.Module):
    def __init__(self, config, num_labels, num_mesh_labels, temp=0.05, clip=0.95):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(config.bert_model_name, do_lower_case=True, use_fast=True)
        if config.use_entity_type_marker:
            print("INFO: Adding suggested special tokens to the tokenizer...")
            special_tokens_dict = {"additional_special_tokens": ["[disorder]","[chemical]","[anatomy]","[physiology]","[gene]","[geographic]"]}
            num_added = self.tokenizer.add_special_tokens(special_tokens_dict)
            print(f"INFO: Added {num_added} special tokens.")
            for tok in self.tokenizer.additional_special_tokens:
                print(tok, "→", self.tokenizer.convert_tokens_to_ids(tok))

            self.bert.resize_token_embeddings(len(self.tokenizer))
            print(f"BEFORE: Model embeddings has {len(self.tokenizer)} tokens.")
            ...
            for special, anchor in anchor_map.items():
                special_id = self.tokenizer.convert_tokens_to_ids(special)
                anchor_id = self.tokenizer.convert_tokens_to_ids(anchor)

                if anchor_id != self.tokenizer.unk_token_id:
                    emb.weight.data[special_id] = emb.weight.data[anchor_id].clone()
                    print(f"Initialized {special} (id={special_id}) from '{anchor}' (id={anchor_id})")
                else:
                    print(f"Warning: '{anchor}' not found in vocab, leaving {special} random.")
                        
            print(f"AFTER: Model embeddings resized to {len(self.tokenizer)} tokens.")

        if config.adversarial:
          self.num_mesh_labels = num_mesh_labels 
          print("INFO: Adding adversarial head for domain label prediction...")
          if config.advanced_stronger_adv:
            self.adv_head = nn.Sequential(
              nn.LayerNorm(self.bert.config.hidden_size),
              nn.Linear(self.bert.config.hidden_size, 512),
              nn.ReLU(),
              nn.Dropout(self.config.bert_dropout),
              nn.Linear(512, self.num_mesh_labels)
            )
          else:
              self.adv_head = nn.Linear(self.bert.config.hidden_size, self.num_mesh_labels)
    def forward(self, batch, training=False, sup_contrastive_mode="HeroCon", focusing=0, clipping=0):
        ...
        return [self.alpha * l2, self.beta * l3], logits, adv_logits # cl_losses, logits, adv_logits
